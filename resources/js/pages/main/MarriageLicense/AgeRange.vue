...
export default {
  ...
  data() {
    return {
      ...
      isForeigner: null,
      config: {
        servicesHeading: "What is your age range?",
        servicesSubtext: "select appropriate age range for requirements."
      },
      
      services: [
        { title: "18-21 years old", emoji: "📝", description: "Parental Consent.", gradient: "linear-gradient(135deg, #1e3c72 0%, #2a5298 100%)", value: "parental-consent" },
        { title: "21 or over to below 25 years old", emoji: "🗣️", description: "Parental Advise.", gradient: "linear-gradient(135deg, #1e3c72 0%, #2a5298 100%)", value: "parental-advise" },
        { title: "25 years old or over ", emoji: "✨", description: "No requirements.", gradient: "linear-gradient(135deg, #ff6a00 0%, #ee0979 100%)", value: "no-need" }
      ]
    };
  },
  methods: {
    navigateTo(value) {
      const link = `/Marriage-Form/ageRange/requirements/${value}/isForeigner/${this.$route.params.isForeigner}/Cohab/${this.IsCohabiting}`;
